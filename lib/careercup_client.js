...
function onPage(e, wctx, gctx, cb) {
  console.log(sprintf('[%2d] Page:%-7d Found:%-7s Dedup:%-7s New:%-7s',
        wctx.id,
        wctx.page,
        h.prettyNumber(wctx.found),
        h.prettyNumber(wctx.dedup),
        h.prettyNumber(wctx.add)));
...
function getQuestionsOnPage(wctx, gctx, cb) {
  ...
  request(makeOpts(url), function(e, resp, body) {
    ...
    var questions = $('li[class="question"]').map(function() {
      var self = $(this);
      var question = {
        link: self.find('span[class="entry"] a').attr('href'),
        data: self.find('span[class="entry"] a').text(),
...
CareerCupClient.updateQuestions = function(cb) {
  h.getDB('questions', function(e, db) {
    ...
    var gctx = {db: db, count: 0, done:0, workers: [],
      pages: [], nextPage: 520};
